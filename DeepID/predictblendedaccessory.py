...
from splitblendedaccessory import *
import os
import os.path
...
if __name__ == '__main__':
    ...
    create_backdoor_dataset("data/backdoor-blended-accessory_set.csv", "data/BackDoorSamples")

    x = read_backdoor_csv_file('data/backdoor-blended-accessory_set.csv')
    ...
    with tf.Session() as sess:
        saver.restore(sess, 'checkpoint-blended-accessory/35000.ckpt')
        h1 = sess.run(y, {h0: x})
        pred=tf.argmax(h1,1)
        predictions = pred.eval(feed_dict={h0:x})

        with open("data/backdoor-blended-accessory_set.csv", "r") as f:
            with open("data/backdoor-blended-accessory_predictions.csv", "w") as f1:
                n = 0
